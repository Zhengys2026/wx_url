...
namespace Utilities
{
    public class ImageClass
    {
        ...
        public EntityImage MakeThumbnail(string originalImagePath, string thumbnailPath, int width, int height, string mode, string fileExtension)
        {

            EntityImage eimg = new EntityImage();
        ...
        /// <param name="height">以多少高度进行切割</param>
        /// <param name="outputFile">输出文件名，不带后缀</param>
        /// <returns></returns>
        public int imageHeightCut(string activity_id, FileInfo sourceFile, int cut_height, String outputFile)
        {
            try
            {
                long imageQuality = 25L;
                Bitmap sourceImage = new Bitmap(sourceFile.FullName);
                ...
                //图片横向分割
                float yNum = int.Parse((sourceImage.Height / cut_height).ToString());
                ...
                for (int countY = 0; countY < yNum; countY++)
                    for (int countX = 0; countX < xNum; countX++)
                    {
                        ...
                        outputImage = outputFile + ".c" + countY + sourceFile.Extension;
                ...
                //修正程序内 的数据
                //Business.BusinessActivityInfo _activity = new Business.BusinessActivityInfo();
                //_activity.updateCutImage(activity_id, int.Parse(yNum.ToString()));
                return int.Parse(yNum.ToString());
            }
            catch
            {
                return 0;
